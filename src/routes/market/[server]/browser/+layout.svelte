  ...
  <div class="flex gap-2">
    {#each ['Buy', 'Sell'] as tab}
      <a
        ...
        href="/market/{$page.params.server}/browser/{tab.toLowerCase()}/{$page.params.category}/1{$page.url.search}">
    ...
    <div class="row-span-full grid min-h-0 w-full grid-rows-[subgrid]">
      <div class="row-start-1 flex flex-nowrap place-items-center justify-between">
        ...
        <div class="flex place-items-center gap-2 px-2">
          <button class="" disabled={+$page.params.page == 1}>
            <a
              href={$page.route.id
              ...
              <img
                ...
                alt=""
                class="rotate-180"
                class:opacity-30={+$page.params.page === 1} />
            </a>
          </button>
          Page {$page.params.page} of {$endPage}
          <button class="" disabled={+$page.params.page >= $page.data.marketdata.end}>
            <a
              href={$page.route.id
                ?.replace('[server]', $page.params.server)
                .replace('[category]', $page.params.category)
              ...
              <img
                ...
                alt=""
                class=""
                class:opacity-30={+$page.params.page >= $page.data.marketdata.end} />
            </a>
          </button>
        </div>
      </div>
