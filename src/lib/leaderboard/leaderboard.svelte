<script lang="ts">
  ...
  import type { LeaderboardDefinition, LeaderboardIdMap, LeaderboardType } from './leaderboardmap'
...
{#if $categories.firstlevelcategory && $categories.category && $categories.subcategory && leaderboards && leaderboards.length > 1}
  <div class="relative flex place-content-center rounded-md lg:hidden">
    <div class="dropdown dropdown-end grow">
      ...
      <label tabindex="0" class="btn btn-xs m-1 md:btn-sm">
        <div class="flex flex-col">
          {#if leaderboardId && leaderboards.find((item) => item.LeaderboardDefinitionId === leaderboardId)?.DisplayName === $categories.subcategory}
            <div class="">
              {leaderboards.find((item) => item.LeaderboardDefinitionId === leaderboardId)?.CharacterLeaderboard
                ? 'Character'
                : leaderboards.find((item) => item.LeaderboardDefinitionId === leaderboardId)?.CompanyLeaderboard
                ? 'Company'
                : ''}
            </div>
          {:else if leaderboardId && leaderboards.find((item) => item.LeaderboardDefinitionId === leaderboardId)?.DisplayName !== $categories.subcategory}
            {#if leaderboards.find((item) => item.LeaderboardDefinitionId === leaderboardId)}
              {leaderboards.find((item) => item.LeaderboardDefinitionId === leaderboardId)?.DisplayName}
            {:else}
      ...
      <ul
        tabindex="0"
        class="min-h-24 small-scrollbar dropdown-content dropdown-end menu rounded-box max-h-[35vh] w-52 flex-nowrap overflow-y-auto overflow-x-hidden bg-base-100 p-2 shadow">
        {#each Object.values(leaderboards) as categoryKeys, i}
          <li>
            <button
              class={leaderboardId === categoryKeys.LeaderboardDefinitionId ? 'active' : ''}
              on:pointerup={(e) => {
                if (e.button !== 0) {
                  return
                }
                goto(`/lb/${categoryKeys.LeaderboardDefinitionId}/${data.currentSeason}`)
              }}>
              <div class="flex">
              ...
              {#if categoryKeys.CharacterLeaderboard || categoryKeys.CompanyLeaderboard}
                <div
                  class="tooltip tooltip-info px-0 {i == 0 ? 'tooltip-bottom' : ''}"
    ...
    <div
      class="absolute bottom-0 -z-10 h-[5%] w-[100%] grow {leaderboardIdMap[leaderboardId]?.Category ===
        $categories.category && leaderboardIdMap[leaderboardId].SecondLevelCategory === $categories.subcategory
        ? 'bg-accent'
        : ''} " />
  </div>
{/if}
